...
<div class="mx-auto grid min-h-screen max-w-5xl items-center gap-6 p-4 lg:grid-cols-2">
    ...
    <section class="space-y-5">
        @if(session('success'))
            <div class="rounded-xl bg-emerald-100 p-3 text-emerald-900">{{ session('success') }}</div>
        @endif
        ...
        <form method="POST" action="{{ route('login.store') }}" class="space-y-4 rounded-3xl bg-white p-6 shadow-sm">
            @csrf
            <h2 class="text-xl font-bold">ورود با رمز عبور</h2>
            <label class="block">
                <span>شماره موبایل</span>
                <input name="mobile" value="{{ old('mobile') }}" class="mt-1 w-full rounded-xl border p-3" dir="ltr" required>
            </label>
            <label class="block">
                <span>رمز عبور</span>
        ...
        <form method="POST" action="{{ route('login.otp.request') }}" class="space-y-4 rounded-3xl bg-white p-6 shadow-sm">
            @csrf
            <h2 class="text-xl font-bold">ورود با کد یک‌بارمصرف</h2>
            <label class="block">
                <span>شماره موبایل</span>
                <input name="mobile" value="{{ session('otp_mobile', old('mobile')) }}" class="mt-1 w-full rounded-xl border p-3" dir="ltr" required>
            </label>
            <button class="w-full rounded-xl bg-blue-600 p-3 text-white">ارسال کد</button>
        </form>

        <form method="POST" action="{{ route('login.otp.verify') }}" class="space-y-4 rounded-3xl bg-white p-6 shadow-sm">
            @csrf
            <h2 class="text-xl font-bold">تایید کد ورود</h2>
            <input name="mobile" value="{{ session('otp_mobile', old('mobile')) }}" class="w-full rounded-xl border p-3" dir="ltr" placeholder="شماره موبایل" required>
